shinyServer(function(input, output,session) {
  ...
  #Body2 : Border Information ####
  output$plot1 <- renderPlot(
    borderF %>% 
      group_by(Border) %>% 
      summarise(NumberPort = n_distinct(Port.Name)) %>% 
      ggplot(aes(Border, NumberPort)) + geom_col(fill='light pink') + labs(x="Border", y="Number of Port")
  )
  ...
  output$plot1_2 <- renderPlot(
    borderF %>% 
      ...
      ggplot(aes(Border, totalIncBor)) + geom_col(fill='orange') + labs(x="Border", y="Number of Incoming")
  )
